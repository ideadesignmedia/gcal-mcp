...
export function all<T = any>(db: DB, sql: string, params: any[] = []): Promise<T[]> {
  return new Promise((res, rej) => db.all(sql, params, (err, rows) => err ? rej(err) : res(rows as T[])));
}
export async function tx<T>(db: DB, fn: () => Promise<T>): Promise<T> {
  await run(db, 'BEGIN IMMEDIATE');
  try { const out = await fn(); await run(db, 'COMMIT'); return out; }
  catch (e) { try { await run(db, 'ROLLBACK'); } catch {} throw e; }
}
