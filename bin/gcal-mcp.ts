...
program
  .name('gcal-mcp')
  .description('Private Google Calendar MCP server and account linker')
  .option('--db <path>', 'Path to SQLite database')
  .option('--pass <pass>', 'Database password (or use GMAIL_MCP_DB_PASS)')
  .option('--read-only', 'Disable create/update/delete tools', false);
...
program
  ...
  .action(async () => {
    ...
    if (locked) {
      if (!pass) { console.error('Database is locked. Provide --pass or set GMAIL_MCP_DB_PASS.'); process.exit(11); }
      try { const uw = await tryUnwrapDek(db, pass); dek = uw.dek; }
      catch { console.error('Password is incorrect or database is corrupt.'); process.exit(11); }
    }
...
program
  .command('add')
  .description('Link a Google Calendar account via OAuth')
  ...
  .action(async (opts) => {
    ...
    if (!email || !googleUserId) { console.error('Failed to fetch user profile email and id'); process.exit(3); }
...
program
  ...
  .action(async (key) => {
    ...
    await tx(db, async () => { await run(db, 'DELETE FROM accounts WHERE id=? OR email=?', [key, key]); });
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.rotate || locked) {
      if (!opts.rotate) { opts.rotate = true; (opts as any).oldPass = pass; }
      if (!opts.oldPass) { console.error('Rotation requires --old-pass'); process.exit(2); }
      const newPass = flags.pass || process.env.GMAIL_MCP_DB_PASS;
      if (!newPass) { console.error('Provide new password with --pass or GMAIL_MCP_DB_PASS for rotation'); process.exit(2); }
      await rotatePassword(db, opts.oldPass, newPass, opts.hint);
      console.log('Password rotated');
    } else {
      const newPass = flags.pass || process.env.GMAIL_MCP_DB_PASS;
      if (!newPass) { console.error('Provide password with --pass or GMAIL_MCP_DB_PASS to lock the database'); process.exit(2); }
      await lockDatabase(db, newPass, opts.hint);
      console.log('Database locked');
    }
  });

program.parseAsync().catch(e => { console.error(e?.message || e); process.exit(1); });
